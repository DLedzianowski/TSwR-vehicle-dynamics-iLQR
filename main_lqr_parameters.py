...
def compute_v_ref(kappa, vehicle):
    a_lat_max = 0.4 * vehicle.a_lat_max
    ...
    def optimize(self, x0, u_init, max_iters=10):
        ...
        for it in range(max_iters):
            ...
            for alpha in [1.0, 0.5, 0.25, 0.1]:
                ...
                print(f"iter {it}, "f"u norm {torch.norm(u_new_seq)}, "f"cost = {new_cost}")
...
Alpha = [0.3, 0.7, 1, 1.3, 2.0]
i = 0
for j in range(3):
    for Alpha_ in Alpha:
        params = parameters_vehicle1()
        vehicle = VehicleModel(params)       
        params_iLQR  = parameters_vehicle1()
        if j == 0:
            params_iLQR.m *= Alpha_
        if j == 1:
            params_iLQR.I_z *= Alpha_
        if j == 2:
            params_iLQR.tire.p_ky1 *= Alpha_
            params_iLQR.tire.p_cy1 *= Alpha_
            params_iLQR.tire.p_dy1 *= Alpha_
        vehicle_iLQR = VehicleModel(params_iLQR)
        ...
        max_iters = 10
        N = 10
        v_ref = params.longitudinal.v_max
        dt = 0.05
        Q = np.diag([ # error penalty
            0.0,     # s
            3000.0,  # n
            500.0,   # μ
            50.0,    # vx
        ...
        R = np.diag([1, 0.1]) # effort penalty
        Qf = Q * 5 # kara za stan koncowy
        ...
        x_ref_seq = get_x_ref_seq(ref_path, vehicle_iLQR, x[0].item(), N, dt)
        ilqr = ILQRController(vehicle_iLQR, Q, R, Qf, N, dt, kappa_seq, x_ref_seq, nx, nu)
        ...
        try:
            for t in range(100): # 300
                ...
                try:
                    ...
                    u_init = torch.vstack([u_opt[1:], u_opt[-1:]]) # warm start
                    print("error: ", x[1])
                    n_history.append(x[1].item())
        ...
        metrics = {
            "alpha": Alpha_,
            "mean_abs": np.mean(np.abs(n_array)),
            "rms": np.sqrt(np.mean(n_array**2)),
            "max_abs": np.max(np.abs(n_array)),
            "final_error": n_array[-1],
            "steps": len(n_array),
            "distance": x[0].item()
        }

        if j == 0:
            results["mass"].append((metrics, n_array))

        elif j == 1:
            results["Iz"].append((metrics, n_array))

        elif j == 2:
            results["tire"].append((metrics, n_array))
...
for ax, key, title in zip(axs, keys, titles):
    ...
    for metrics, n_array in results[key]:

        ax.plot(
            n_array,
            label=f"alpha={metrics['alpha']}"
        )
...
for key in results:
    ...
    for metrics, _ in results[key]:

        print(
            f"alpha={metrics['alpha']:.2f} | "
            f"mean={metrics['mean_abs']:.3f} | "
            f"rms={metrics['rms']:.3f} | "
            f"max={metrics['max_abs']:.3f} | "
            f"final={metrics['final_error']:.3f} |"
            f"dist={metrics['distance']:.1f} "
        )